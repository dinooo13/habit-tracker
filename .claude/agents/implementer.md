...
3. Conventions: `<script setup>` + Composition API; local `YYYY-MM-DD` date keys and
   `HH:MM` times (`app/utils/domain/date.ts`); IDs via `createId(prefix)`; everything loaded or
   imported is Zod-validated (`app/utils/persistence/storage-schema.ts`).
...
- If a review comment (`<!-- routine:code-review sha=… -->`), a QA comment
  (`<!-- routine:qa sha=… -->`), or a rebaser bounce comment (`<!-- routine:rebase -->`)
  reported blocking findings, those findings are your work queue — the **Blocking**
  lists for review/QA; for a rebase bounce: rebase onto `origin/main`, resolve the
  conflicts (or the failing gates) using the plan as scope authority, and re-run the
  gates. Otherwise continue from the first unchecked task.
